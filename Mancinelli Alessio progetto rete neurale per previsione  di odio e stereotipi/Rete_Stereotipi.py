...
# Filtro ed ottengo solamente le colonne da dare in input al modello inlusa la colonna stereotipi
...
#Ottengo i valori dla dataframe
...
model_final = Dense(32, activation = "relu", bias_initializer='zeros') (x)
model_final = Dense(2, activation='softmax', bias_initializer='zeros')(model_final)
model_final = Model(input_testo, model_final)
model_final.compile(loss="categorical_crossentropy", optimizer = 'adam', metrics = ["accuracy"])


plot_model(model_final,to_file="model_plot_stereotipi.png", show_shapes = True, show_layer_names = True)
...
model_final.save_weights("Modello_Stereotipi_Salvato/modello_stereotipi.h5")
